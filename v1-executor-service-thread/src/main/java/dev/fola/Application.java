...
public class Application {
    ...
    public static void main(String[] args) {
        System.out.println("=== v1 - Executor Service  Importer (with timing loop) ===\n");
        ...
        System.out.printf("Available System processor is %d",THREAD_POOL_SIZE);
        ...
        for (int run = 1; run <= WARMUP_RUNS + MEASUREMENT_RUNS; run++) {
            ...
            for (String email : mockEmails) {
                Future<Expense> future = executorService.submit(()-> ExpenseParser.emailParser(email));
                futures.add(future);
            }
